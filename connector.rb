{
  ...
  connection: {
    fields: [
      ...
      #   hint: 'Defines how to provide the OpenAPI document to the connector. ' \
      #         'The preferred approach is to load it from a URL. ' \
      #         'Otherwise, copy and paste the contents of the file. ' \
      #         '<a target="_blank" href="https://docs.workato.com/connectors/openapi/' \
      #         'guides/setting-up-the-openapi-connector.html#provide-api-definition">' \
      #         'Learn more</a>',
      #   optional: false,
      #   control_type: 'select',
      #   options: [
      #     %w[Document\ URL url],
      #     %w[Document\ content content]
      #   ]
      # },
      # {
      #   name: 'definition_url',
      #   label: 'Document URL',
      #   hint: 'This URL must be publicly accessible at all time. ' \
      #         'Any future changes to the document itself appear automatically ' \
      #         'in the Workato recipe editor. ' \
      #         'The document can be in either JSON or YAML format. ' \
      #         'Supports OpenAPI v2 and v3.',
      #   optional: false,
      #   control_type: 'url',
      #   ngIf: 'input.definition_mode == "url"'
      # },
      # {
      #   name: 'definition_content',
      #   label: 'Document content',
      #   hint: 'Content of the OpenAPI document. ' \
      #         'Format is either JSON or YAML. ' \
      #         'Supports OpenAPI v2 and v3.',
      #   optional: false,
      #   control_type: 'plain-text-area',
      #   ngIf: 'input.definition_mode == "content"'
      # },
      {
        name: 'gcp_bucket',
        label: 'Bucket',
      ...
      },
      {
        ...
        options: [
          ...
          # %w[Basic basic],
          %w[Header header],
          # %w[Query\ params query],
          # %w[OAuth\ 2.0\ client\ credentials oauth2_client_credentials],
          # %w[OAuth\ 2.0\ authorization\ code oauth2_authorization_code],
          # %w[OAuth\ 2.0\ resource\ owner\ password oauth2_resource_owner_password]
        ]
      },
      {
        name: 'base_url',
    ...
    authorization: {
      ...
      options: {
        ...
        query: {
          ...
          fields: [
            {
              ngIf: '!(input.query_params == null || input.query_params == "")',
              name: 'query_params',
              label: 'Query param authorization (Deprecated)',
              hint: 'Add custom query params, one per line, e.g. <b>token=secret42</b>',
              optional: false,
              type: :string,
              control_type: 'text-area',
              multiline: true
            ...
            {
              ngIf: 'input.query_params == null || input.query_params == ""',
              name: 'query_params_key_value',
              label: 'Query param authorization',
              control_type: 'key_value',
              empty_list_title: 'Does the application require query param authorization?',
              empty_list_text: 'Refer to the API documentation and add ' \
              'required query params.',
              item_label: 'Query Params',
              type: 'array',
              of: 'object',
              optional: false,
              properties: [
                { name: 'key', label: 'Query param name' },
                { name: 'value', control_type: 'password' }
              ]
            },
            # rubocop:disable Layout/LineLength
          ...
          apply: lambda do |connection|
            ...
            unless skip_auth
              params_input_string = connection['query_params']
              params_input_key_value = connection['query_params_key_value']
              if params_input_string.present?
                params_input_lines = params_input_string.split(/\n+|\r+/)
                params_object = {}
                params_input_lines.each do |line|
                  next if line.blank?
                  ...
                  params_object[name] = value
                end
                params(params_object)
              elsif params_input_key_value.present?
                key_value_hash = params_input_key_value&.each_with_object({}) do |item, hash|
                  hash[item['key']] = item['value']
                end || {}
                params(key_value_hash)
              end
            end
          end
        },
        ...
        oauth2_client_credentials: {
          ...
          fields: [
            ...
            {
              name: 'access_token_request_mode',
              label: 'How does the API require credentials to be sent to request a token?',
              optional: true,
              hint: 'Send client ID and secret in token request body or ' \
                    'as a base64 encoded string in the header?',
              control_type: :select,
              options: [
                %w[Body body],
                %w[Header header]
            ...
            },
            {
              ...
              label: 'Client ID',
              optional: false,
              hint: 'Public ID of the OAuth app that should be tied to Workato. ' \
                    'This might require signing Workato up as a verified application.'
            },
            {
              name: 'client_secret',
              label: 'Client secret',
              optional: false,
              control_type: :password,
              hint: 'Matching private key that the API will verify along with the Client ID. ' \
                    'This might mean signing Workato up as a verified application. ' \
                    '<b>Never share your client secret with others</b>'
            },
            {
              name: 'scopes',
              label: 'OAuth2 scopes',
              ...
              multiline: true,
              control_type: 'text-area',
              hint: 'Scope for access request. ' \
                    'Separate each value with a space or newline'
            },
            {
              name: 'additional_params',
              label: 'Additional OAuth2 parameters',
              optional: true,
              multiline: true,
              control_type: 'text-area',
              hint: 'Any custom name-value pairs, one per line, ' \
                    'e.g. <b>audience: https://api.yourcomp.com</b><br>' \
                    'These will be use only to get the auth token.'
            },
            # rubocop:disable Layout/LineLength
            {
              name: 'advanced',
          ...
          acquire: lambda do |connection|
            request = post(connection['token_url'])
            request_mode = connection['access_token_request_mode'] || 'header'
            scopes = connection['scopes']
            scopes = scopes&.split(/\n+|\r+/)
            scopes = scopes&.map { |scope| scope&.split(' ') }
            scopes = scopes&.flatten&.compact&.uniq
            scopes = scopes&.join(' ')
            ...
            additional_params&.each do |param|
              ...
              name = param[0..(i - 1)]&.strip
              value = param[(i + 1)..(param.length - 1)]&.strip
              next if name.blank?
              next if value.blank?
            ...
            when 'header'
              request = request.user(connection['client_id'])
              request = request.password(connection['client_secret'])
            end
        ...
        oauth2_authorization_code: {
          ...
          fields: [
            {
              ...
              optional: false,
              hint: 'Use <b>https://www.workato.com/oauth/callback</b> as callback URL ' \
                    'when creating the OAuth 2.0 client in the application.'
            },
            {
              name: 'token_url',
              label: 'OAuth2 token URL',
          ...
          authorization_url: lambda do |connection|
            ...
            scopes = scopes&.join(' ')
            if scopes.present?
              "#{connection['authorization_url']}?response_type=code&scope=#{scopes.encode_url}"
            else
              "#{connection['authorization_url']}?response_type=code"
            end
          end,
          ...
          acquire: lambda do |connection, auth_code, redirect_uri|
            request = post(connection['token_url'])
            request_mode = connection['access_token_request_mode'] || 'body'
            payload = {
      ...
      config_fields: [
        ...
        # },
        {
          ...
          hint: 'This URL must be publicly accessible at all time. ' \
                ...
                'The document can be in either JSON or YAML format. ' \
                'Supports OpenAPI v2 and v3.',
          control_type: 'url', support_pills: false,
          ngIf: 'input.definition_mode == "url"'
        },
        {
          name: 'definition_content',
          ...
          hint: 'Content of the OpenAPI document. ' \
                'Format is either JSON or YAML. ' \
                'Supports OpenAPI v2 and v3.',
          control_type: 'plain-text-area', support_pills: false,
          ngIf: 'input.definition_mode == "content"'
        },
        # {
        #   name: 'base_url', label: 'Server URL', control_type: 'url',
        #   hint: 'A URL to the target host or service, ' \
        #         'e.g. <b>https://app.example.com</b> or <b>https://example.com/app</b>. ' \
        #         'Relative endpoint paths from the OpenAPI document will be appended ' \
        #         'to the Server URL in order to construct the full endpoint URL.'
  ...
  methods: {
    ...
    adjust_connection: lambda do |connection, input|
      if input[:definition_mode] == 'url' && input[:definition_url].present?
        connection['definition_mode'] = input[:definition_mode]
        connection['definition_url'] = input[:definition_url]
      elsif input[:definition_mode] == 'content' && input[:definition_content].present?
        connection['definition_mode'] = input[:definition_mode]
        connection['definition_content'] = input[:definition_content]
      end
    ...
    get_api_definition: lambda do |connection|
      unless connection.has_key?('spec')
        definition_url = connection['definition_url']
